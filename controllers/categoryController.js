...
static getAllCategory = async (req, res, next) => {
    try {
        ...
        JSONResponse.error(res, "Error while retrieving categories", error, 400);
    }
};
...
static updateCategory = async (req, res, next) => {
    ...
    static deleteCategory = async (req, res, next) => {
        try{
            ...
            JSONResponse.success(
                res, 
                "Category has been deleted successfully",category,200
            );
        } catch (error) {
    ...
    static getCategoryById = async (req, res, next) => {
        try{
            let id = req.params.id;
            if (!ObjectId.isValid(id))
                throw new Error("CategoryID isnt valid");
